...
bool test_quantization(const string& audio_file, int quant_bits, TestResult& result) {
    ...
    string enc_cmd = "./wav_dct_enc -qbits " + to_string(quant_bits) + " " + escape_shell(audio_file) + " " + escape_shell(encoded_file) + " " + to_string(quant_bits) + " > /dev/null 2>&1";
    ...
    string dec_cmd = "./wav_dct_dec " + escape_shell(encoded_file) + " " + escape_shell(decoded_file) + " > /dev/null 2>&1";
